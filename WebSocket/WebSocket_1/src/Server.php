...
class Server
{
    ...
    public function __construct($host, $port)
    {
        try {
            ...
            // listen 函数使用主动链接套接口变为被链接套接口，使得一个进程可以接受其他进程的请求，从而成为一个服务器进程
    ...
    {
        $write = $except = NULL;
        $sockets = array_column($this->sockets, 'resouce');
        $readNum = socket_select($sockets, $write, $except, NULL);

        // select 作为监视函数，参数分别是(监视可读、可写、异常、超时时间)，返回可操作数目，出错时返回false
        if (false == $readNum) {
            $this->error([
                'errorSelect',
                $errCode = socket_last_error(),
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                array_unshift($recvMsg, 'receive_msg');
    ...
    public function handShake($socket, $buffer)
    {
        ...
        $upgradeMessage .= "Sec-WebSocket-Accept:" . $upgrade_key . "\r\n\r\n";
