...
def train(fileName):
    ...
    在该函数中，我们需要通过给定的训练数据(包含S个长度相同的观测序列【写一句还】和对应的状态序列【每一句话中每个词的词性】)
    ...
    with open(fileName, encoding = 'utf-8') as file:
        ...
        语料库为我们进行了切分，每一个词语用空格隔开
        那么在这其中，我们将每个切分(包括标点符号)放在列表中
        然后遍历列表每一个元素
        当列表词语长度为1的时候，如'的'字，那么我们就认为状态S(单个字)
        ...
        for line in file.readlines():
            ...
            words = line.strip().split()    # 除去前后空格，然后依照中间空格切分单词
            ...
            for i, word in enumerate(words):
                ...
                if len(word) == 1:
                    ...
                    # 使用ord找到该字对应编码
                    # 更新B矩阵
                    # B代表了每一个状态对应的观测结果的可能性
                    # 先统计频数
                    for s in range(len(word)):
                        code = ord(word[s])
                        B[status2num[status[s]]][code] += 1

                # i == 0意味着这是句首。需要更新PI中每种状态出现次数
                if i == 0:
                    # status[0]表示这行第一个状态
