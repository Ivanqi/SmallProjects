...
func NewGenerator(pset ParamSet)(lib.Generator, error) {
	logger.Inofln("New a load generator...")
...
func (gen *myGenerator) printIgnoredResult(result *lib.CallResult, cause string) {
	resultMsg := fmt.Sprintf("ID=%d, Code=%d, Msg=%d, Elapse=%v", result.ID, result.Code, result.Msg, result.Elapse)
	logger.Warnf("Ignored result: %s. (cause: %s)\n", resultMsg, cause)
}

// prepareStop 用于为停止载荷发生器做准备
func(gen *myGenerator) prepareToStop(ctxError error) {
	logger.Inofln("Prepare to stop load generator (cause: %s)...", ctxError)
...
func (gen *myGenerator) Start() bool {
	logger.Inofln("Starting load generator...")
	...
	// 初始化上下文和取消函数
	gen.ctx, gen.cancalFunc = context.WithTimeout(context.Background(), gen.durationNS)
	...
	go func () {
		// 生成并发送载荷
		logger.Inofln("Generating loads...")
