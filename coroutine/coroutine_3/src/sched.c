...
asmlinkage void __switch_stack(co_t *prev, co_t *next) {
    ...
    if (next->autostack && co_onstack != next) {
        ...
        if (co_onstack) {
            ...
            if (likely(stack_size <= COPY_STACK && !co_onstack->mmapstack)) {
                // 1)
        ...
        if (likely(stack_size <= COPY_STACK && !next->mmapstack)) {
            // 2)
            stack_size = (unsigned long)co_stack_bottom - next->rsp;
            ...
            if (ptr == MAP_FAILED && ptr != co_stack_bottom - next->stack_size) {
...
int co_key_delete(int key) {
    if (unlikely(key < 0 || co_specific_num <= key || !co_specific[key].used)) {
        return EINVAL;
    }
