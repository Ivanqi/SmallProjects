...
namespace mvcc11
{
    ...
    struct snapshot
    {
        ...
        template <class U>
        snapshot(size_t ver, U&& arg) MVCC11_NOEXCEPT(MVCC11_NOEXCEPT(value_type {std::forward<U>(arg)}));
    ...
    template <class ValueType>
    template <class U>
    snapshot<ValueType>::snapshot(size_t ver, U&& arg) MVCC11_NOEXCEPT( MVCC11_NOEXCEPT(value_type{std::forward<U>(arg)}) )
        :version{ver}, value{std::forward<U>(arg)}
    {
    ...
    auto mvcc<ValueType>::update(Updater updater) -> const_snapshot_ptr
    {
        while (true) {
            ...
            std::this_thread::sleep_for(std::chrono::microseconds(MVCC11_CONTENSION_BACKOFF_SLEEP_MS));
        }
    }
